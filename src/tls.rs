...
/// The one file in the data directory that is a secret in its own right: whoever reads it can
/// impersonate the relay to every desktop that pinned its fingerprint.
...
fn load_or_generate(data_directory: &Path, names: &SubjectNames) -> Result<PemMaterial, String> {
    ...
    paths::restrict_to_owner(&directory);

    let files = MaterialPaths::new(&directory);
    let material = match files.read() {
        Some(material) => material,
        None => {
            let material = generate(names)?;
            files.write(&material)?;
            material
        }
    };
    // Narrowed whether it was just written or read back: a key generated by an earlier version sits
    // there at the umask's mercy until something takes the other accounts' access away.
    paths::restrict_to_owner(&files.private_key);
    Ok(material)
}
